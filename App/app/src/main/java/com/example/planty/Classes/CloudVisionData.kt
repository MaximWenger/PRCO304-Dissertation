...
class CloudVisionData {
    //Used to confirm and sort the data, returned by Google Cloud Vision
    private val toRemoveLibrary = arrayOf(
        "Petal",
        "Plant",
        "Yellow",
        "Flower",
        ...
        "Perennial Plant",
        "Annual Plant"
    ) //Used to hold strings of identification labels to be removed
    private val toConfirmPlantLibrary = arrayOf(
        "Petal",
        "Plant",
        "Flower",
        "Flowering Plant",
        "Wildflower"
    ) //Used to confirm the identification is a plant
    ...
    private fun compareLabel(
        text: String,
        type: Int
    ): Boolean { //Returns Boolean if the given string is within the existing blacklist (library)
